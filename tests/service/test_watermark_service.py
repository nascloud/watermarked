...
class TestWatermarkService(unittest.TestCase):
    ...
    def test_create_preview(self, mock_apply_watermark, mock_image_open):
        """测试创建预览的功能"""
        # 模拟图片和水印
        mock_image = Image.new('RGB', (100, 100), 'white')
        mock_watermark = Image.new('RGBA', (10, 10), (255, 0, 0, 255))
        mock_image_open.side_effect = [mock_image, mock_watermark]
        
        # 模拟 apply_watermark 返回一个新图片
        mock_final_image = Image.new('RGBA', (100, 100), (255, 255, 255, 255))
        ...
        mock_image_open.assert_any_call("dummy_path.jpg")
        mock_apply_watermark.assert_called_once_with(
            mock_image, mock_watermark, self.config_data['opacity']
        )
